"""Initial schema — all 28 tables, partitioning, HNSW/BRIN indexes, matview, seeds.

This migration is HAND-WRITTEN (not autogenerated) because it contains:
  - native RANGE partitioning (health_metrics, agent_actions)
  - an HNSW vector index (agent_memory.embedding)
  - a BRIN index (health_metrics.recorded_at)
  - a materialized view (daily_health_summary)
None of which `alembic revision --autogenerate` can express.

The DDL below is the canonical schema, kept in sync with db/schema.sql.
Future schema changes SHOULD use autogenerate against the ORM models.

Revision ID: 0001_initial_schema
Revises:
Create Date: 2026-05-20T13:46:13.432492+00:00
"""
# ruff: noqa: E501  (raw DDL strings — line breaks would harm readability)
...
# Canonical DDL (kept in sync with db/schema.sql).
_UPGRADE_DDL = r"""
-- ============================================================================
...
_DOWNGRADE_DDL = r"""
DROP MATERIALIZED VIEW IF EXISTS daily_health_summary CASCADE;
...
DROP TABLE IF EXISTS workout_suggestions CASCADE;
DROP TABLE IF EXISTS health_markers CASCADE;
...
DROP TABLE IF EXISTS program_split CASCADE;
DROP TABLE IF EXISTS exercise_targets CASCADE;
DROP TABLE IF EXISTS training_context CASCADE;
